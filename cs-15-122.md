...
struct img_header IMG; // <= Error
struct img_header* IMG = alloc(struct img_header); // <= alloc分配内存

// 或者为了方便使用可以这样定义
typedef struct img_header image;
...
// typedef ____* ssa_t;
ssa_t ssa_new(int n);
string get_ssa(ssa_t A, int x);
void set_ssa(ssa_t A, int x, string s);
```

* ssa\_t需要d实现的功能主要有三个，创建新个体，插入元素和读取元素
