...
```c
alloc_array(type, n)//@requires n >= 0;//@ensures \length(\result) == n;
```
...
```c
// A mysterious functionint f(int x, int y){  int r = 1;  while (y > 1) {    if (y % 2 == 1) {      r = x * r;    }    x = x * x;    y = y / 2;  }  return r * x;}
```
...
```c
int f(int x, int y)//@requires y >= 0;                 <= This is how preconditon is implemented.//@ensures \result == POW(x,y);     <= This is how post condition is implemented.{...}
```
...
```c
int f(int x, int y)//@requires y>=0;//@ensures \result==POW(x,y);{...while (e>1)//@loop_invariant POW(b,e)*r;{...}return r*b;}
```
...
```text
$ coin -l util--> 0xAB;171 (int)--> int2hex(171);"000000AB" (string)
```
...
```text
//@requires y!=0;//@requires !(x == int_min() && y == -1); 
```
...
```text
// Example, return the opacity of color Pint opacify(int p) {return}
```
